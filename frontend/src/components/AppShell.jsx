...
const AppShell = ({ title, subtitle, children, actions }) => {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <div className="absolute inset-0 bg-grid bg-[size:48px_48px] opacity-[0.03] pointer-events-none" />
      <div className="relative z-10">
        ...
        <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <motion.div
            ...
            className="mb-8 rounded-3xl border border-gray-200 bg-gray-50 p-6 shadow-glow"
          >
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-neon-500/80">{user?.role || 'user'} workspace</p>
                <h1 className="mt-2 text-3xl font-semibold text-gray-900 sm:text-4xl">{title}</h1>
