...
const LenderDatePicker = ({ name, value, onChange, className = '' }) => {
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      >
        <span className={value ? 'text-white' : 'text-white/35'}>{formatDisplayDate(value)}</span>
      ...
      {open ? (
        <motion.div
          ...
          className="absolute z-50 mt-3 w-[19rem] overflow-hidden rounded-3xl border border-white/10 bg-[#171717] shadow-[0_24px_80px_rgba(0,0,0,0.55)]"
        >
          <div className="border-b border-white/10 px-4 py-4">
            <div className="flex items-center justify-between gap-4">
              <div>
              <p className="text-xs uppercase tracking-[0.25em] text-white/45">Date of birth</p>
              <div className="mt-1 flex items-center gap-2 text-sm font-semibold text-white">
                <span>{monthNames[viewDate.getMonth()]}</span>
                <button
                  type="button"
                  onClick={() => setYearOpen((current) => !current)}
                  className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-sm font-semibold text-white transition hover:border-neon-500/40 hover:text-neon-500"
                >
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => shiftMonths(-1)} className="rounded-full border border-white/10 bg-white/5 p-2 text-white/80 transition hover:border-neon-500/40 hover:text-neon-500">
                <ChevronLeft className="h-4 w-4" />
              </button>
              <button type="button" onClick={() => !isNextMonthDisabled && shiftMonths(1)} disabled={isNextMonthDisabled} className="rounded-full border border-white/10 bg-white/5 p-2 text-white/80 transition hover:border-neon-500/40 hover:text-neon-500 disabled:cursor-not-allowed disabled:opacity-30">
                  ...
                  {yearOptions.map((year) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-xl px-3 py-2 text-sm font-medium transition ${active ? 'bg-neon-500 text-black' : 'text-white/75 hover:bg-white/5 hover:text-white'}`}
                      >
                        {year}
                      </button>
                    );
          ...
          <div className="grid grid-cols-7 gap-1 px-4 pb-2 pt-4 text-center text-[0.65rem] uppercase tracking-[0.25em] text-white/40">
            ...
            {days.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`h-10 rounded-xl text-sm transition ${isFutureDate ? 'cursor-not-allowed text-white/15' : isSelected ? 'bg-neon-500 text-black shadow-lg shadow-neon-500/20' : isCurrentMonth ? 'text-white hover:bg-white/8' : 'text-white/30 hover:bg-white/5'}`}
                >
                  {day.getDate()}
                </button>
              );
          ...
          <div className="flex items-center justify-between border-t border-white/10 px-4 py-3">
            <button type="button" onClick={() => onChange({ target: { name, value: '' } })} className="text-sm font-medium text-white/60 transition hover:text-neon-500">
              Clear
            </button>
